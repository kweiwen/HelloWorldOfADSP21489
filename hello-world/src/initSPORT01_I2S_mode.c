...
void initSPORT()
{
	// Set up "ping-pong" chained DMAs for Codec
    ...
    *pSPCTLN2 = 0;
    ...
    /////////////////////////////////////////////////////////////////////////////////////
	int FSA_DIV = 0x00FF0000;	//8.192MHz/256 = 32K
    int CLKA_DIV = 0x00000006;	//98.304MHz/8 = 12.288MHz （48K采样率）

    // External clock and frame syncs generated by ADSP-21489
   	*pDIV0 = FSA_DIV|CLKA_DIV;  // Transmitter (SPORT0) 设置FS/CLK对应主时钟的分频系数
    *pDIV1 = FSA_DIV|CLKA_DIV;  // Receiver (SPORT1) at 8.192 MHz SCLK and 32 kHz sample rate


	//TCB_Block_A [0] = DMA链表地址
	//TCB_Block_A [1] = 内部缓冲区大小
	//TCB_Block_A [2] = 每次跳转的字节数
	//TCB_Block_A [3] = 内部索引(收/发缓冲区实际地址)
	...
	*pSPCTL1 = OPMODE | SPEN_A | SCHEN_A | SDEN_A | SLEN32 |SPEN_B | SCHEN_B | SDEN_B | IFS; 					//<-- verdert OPMODE, SPEN_A und aus SLEN32-> SLEN24

	// sport0 control register set up as a transmitter in MCM */
	// sport 0 control register, SPCTL0 = 0x000C01F0 */
	*pSPCTL0 = OPMODE | SPEN_A | SCHEN_A | SDEN_A | SPTRAN | SLEN32 |SPEN_B | SCHEN_B | SDEN_B | ICLK | IFS;			//<-- verdert OPMODE, SPEN_A und aus SLEN32-> SLEN24
